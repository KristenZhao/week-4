...
var query6 = _.sortBy(bakedGoods,function(item){return item.inventory;});
...
var query7 = _.groupBy(bakedGoods,function(item2){return item2.type; });
var query7_1 = _.groupBy(bakedGoods,'type'); // this is the shortcut to using functions
console.log('Grouped by type:', query7_1);
...
Whole Wheat ... $4.49
===================== */
var printMenu = function(grouped){_.each(grouped, function(kind) {
    console.log(kind[0].type);
    _.each(kind,function(food){
      console.log(food.name,'...','$'+food.price);
    });
  });
};
